...
private  
  ...
  def get_blog(options)
    blog = EsMediaFile.find(:first,:conditions => ["name=? and media_type = 'blog' and parent_id is null",options['code_blog'].presence||''])
  ...
  def get_breadcrumb(options)
    breadcrumb = EsMediaFile.find(:first,:conditions => ["name=? and media_type = 'breadcrumb' and parent_id is null",options['code_breadcrumb'].presence||''])
  ...
  def get_carousel(options)
    carousel = EsMediaFile.find(:first,:conditions => ["name=? and media_type = 'carousel' and parent_id is null",options['code_carousel'].presence||''])
  ...
  def get_collapse(options)
    collapse = EsMediaFile.find(:first,:conditions => ["name=? and media_type = 'collapse' and parent_id is null",options['code_collapse'].presence||''])
  ...
  def get_image_list(options)
    image_list = EsMediaFile.find(:first,:conditions => ["name=? and media_type = 'image_list' and parent_id is null",options['code_image_list'].presence||''])
