...
class EsUsersController < ApplicationController
 ...
 before_filter :login_required, :except => ["login", "login_connect", "logout", "new_user", "create", "confirm", "password_lost", "password_recovery","confirm_mail"]
  ...
  def create_complete
    ...
    @user.active = 'N'
    if @user.save
      flash[:notice] = "Cet utilisateur a été correctement créée. Vous pouvez maintenant l'activer, le modifier et lui donner des accès."
  ...
  def destroy
      ...
      if tmp.mail == EsUser.current_user.mail
        ...
        unless tmp.destroy
          tmp.errors.full_messages.each do |tmp_error| 
            tmp_element_error.errors.add(:base, "Utilisateur '#{tmp.mail}' : #{tmp_error}. Il a été désactivé")
          end
    ...
    if tmp_element_error.errors.empty? 
      flash[:notice] = 'Utilisateur(s) correctement supprimé(s).'
    else
  ...
  def activation
    ...
    if tmp.mail == EsUser.current_user.mail       
      flash[:error] = "Vous ne pouvez pas vous désactiver vous-même." 
    else
      mail = send_mail_activation(tmp, "")
      tmp.active == 'Y' ? tmp.update_attribute(:active, "N") : tmp.update_attribute(:active, "Y")    
      flash[:notice] = "L'utilisateur '#{tmp.mail}' a été #{tmp.active == 'Y' ? 'activé' : 'désactivé'}." 
  ...
  def profile_update
    @user = EsUser.find_by_id(EsUser.current_user.id)    
    if @user.update_attributes(params[:user]) 
      flash[:notice] = "Votre profil a correctement été modifié."
    ...
    unless user.blank? 
      ...
      if self.current_user
        if self.current_user.active == "Y"
          ...
          flash[:notice] = "Bonjour #{self.current_user.firstname} #{self.current_user.name}"
          ...
          flash[:error] = "L'utilisateur n'est pas actif"
          redirect_to :action => :login
        end
      else
        flash[:error] = "L'utilisateur ou le mot de passe est invalide!"
        redirect_to :action => :login
      end
    else
      flash[:error] = "L'utilisateur est invalide!"
      redirect_to :action => :login
    end
  end
  ...
  def logout
    self.current_user = nil
    reset_session
    flash[:notice] = "L'utilisateur a été déconnecté."
    redirect_to :controller => :sites, :action => :index
  end
  ...
  def create
    ...
    if @user.save
      ...
      flash[:notice] = "L'utilisateur '#{@user.mail}' a été créé. Un mail vient de lui être envoyé."
      redirect_to :controller => :sites, :action => :index
    else
  ...
  def password_update
    ...
    if @user.update_attributes(params[:user]) 
      ...
      flash[:notice] = "Le mot de passe de l'utilisateur '#{@user.mail}' a été modifié. Un mail vous a été envoyé."
      redirect_to :controller => :sites, :action => :index
    else
      @user.oldpassword=""
      @user.password=""
  ...
  def mail_update
    ...
    if @user.update_attributes(params[:user]) 
      ...
      flash[:notice] = "La demande de changement de mail '#{@user.newmail}' est enregistrée. Un mail vous a été envoyé."
      redirect_to :controller => :sites, :action => :index
    else
      @user.newmail=""
      render :action  => 'mail'
  ...
  def confirm_mail
      ...
      unless user.blank?
        ...
        flash[:notice] = "Votre nouvelle adresse mail '#{user.mail}' est maintenant active."
      else
        flash[:error] = "Aucun utilisateur ne correspond à cette activation de mail."
      end
      redirect_to :controller => :sites, :action => :index
  end
  ...
  def confirm
      ...
      unless user.blank?
        ...
        flash[:notice] = "L'inscription de '#{user.mail}' est validée."
      else
        flash[:error] = "Aucun utilisateur ne correspond à cette validation."
      end
      redirect_to :controller => :sites, :action => :index
  end
  ...
  def password_recovery
    ...
    unless user.blank?
      ...
      flash[:notice] = "Un mail vous a été envoyé à l'adresse '#{user.mail}'."
      redirect_to :controller => :sites, :action => :index    
    else
      flash[:error] = "Aucun utilisateur ne correspond à ce mail."
...
private
  ...
  def send_mail_activation(user, activation)
    unless activation == "[mail]"
      ...
      flash[:notice] += " Un mail vient d'être envoyé à l'utilisateur."
    end
